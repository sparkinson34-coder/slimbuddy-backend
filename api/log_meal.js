...
router.post('/', secureRoute, async (req, res) => {
  try {
    ...
    const { date, meal_description, syns, calories, healthy_extra_a_used, healthy_extra_b_used, notes } = req.body || {};
    if (!meal_description) return res.status(400).json({ error:'Missing meal_description' });
    if (syns==null || isNaN(Number(syns))) return res.status(400).json({ error:'Invalid syns' });

    const d = normalizeDate(date) || new Date().toISOString().slice(0,10);
    const entry = {
      user_id, date:d, meal_description,
      syns: Number(syns),
    ...
    if (error) { console.error('[log_meal] DB:', error.message); return res.status(500).json({ error:'Database error' }); }

    res.json({ message:'✅ Meal logged successfully', data });
  } catch (err) { console.error('log_meal exception:', err); res.status(500).json({ error:'Server error' }); }
});
